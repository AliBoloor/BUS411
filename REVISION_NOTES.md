...
- Removed the sentence attributing the learning objectives directly to Chapter 1; retained the Fabozzi chapter in suggested readings. Expanded the roadmap explanation into grouped bullets.
- Replaced the stacked yellow-background securitization graphic with a shared editable SVG and PNG: two white-background, side-by-side diagrams. Stable filenames avoid autogenerated figure collisions.
- Rebuilt Lecture One's deck as 20 slides in the requested teaching sequence, opening with the roadmap and ending with a brief secondary-market overview. Removed the former standalone currency example, spread example and concluding checklist.
- Added the coupon calculation, maturity conventions, a labeled illustrative yield curve, SOFR/CORRA definitions, the existing repo graphic, inverse-floater cash-flow and discounting channels, sector descriptions and the complete lecture risk taxonomy across two slides.
- Corrected the amortization example's wording to match its equal-principal repayment table. No other lectures or private teaching materials were changed. Local render and commit only; no push.
