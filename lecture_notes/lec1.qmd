...
**Read the map from the top down.** Primary markets raise new funding; secondary markets let investors trade existing claims without providing new money to the original borrower. Dealers, trading platforms and settlement infrastructure support these connections. News about monetary policy, inflation and borrower health changes expectations, affecting prices and yields. Those changes feed back into investors’ portfolio values and borrowers’ costs of new financing or refinancing.

**Fixed payments do not imply fixed value.** We study prices, yields and the yield curve, then measure sensitivity using duration, the dollar value of a one-basis-point yield change (DV01), and convexity. Later lectures add interest-rate models, credit risk and embedded options. The braces provide a roadmap, not prerequisites for this introductory lecture.

**Exposure can be chosen and reshaped.** Bond selection, portfolio weights and derivatives can offset an existing risk or express a market view. An appropriately sized short position in bond futures can reduce the rate exposure of a bond portfolio. Buying a conventional fixed-rate bond in anticipation of falling yields instead adds directional exposure, holding other factors constant. Whether a position hedges or speculates depends on the entire balance sheet. Imperfect offsets, leverage and funding needs mean that transferring risk does not eliminate it.
...
```{mermaid}
%%| label: fig-lec1-securitization
%%| fig-width: 6
%%| fig-height: 8
%%| fig-align: center
%%{init: {'theme': 'default', 'flowchart': {'htmlLabels': false}}}%%
flowchart TB
  subgraph funding["Funding at issuance"]
    direction TB
    I["Investors"] -->|"Cash to buy securities"| T["Trust or SPV"]
    T -->|"Cash to buy the loan pool"| L["Lender"]
    L -->|"Transfers existing loans"| T
    T -->|"Issues securities backed by the pool"| I
  end
  subgraph payments["Payments over time"]
    direction TB
    B["Borrowers"] -->|"Interest and principal"| S["Loan servicer"]
    S -->|"Collections after fees"| P["Trust distributes pool cash flows"]
    P -->|"Payments under the security terms"| H["Investors holding the securities"]
  end
  funding ~~~ payments
  classDef participant fill:#EDE9FE,stroke:#8B5CF6,color:#202124,stroke-width:1.5px;
  classDef intermediary fill:#DBEAFE,stroke:#3B82F6,color:#202124,stroke-width:1.5px;
  class I,L,B,H participant;
  class T,S,P intermediary;
  style funding fill:#FFFDE7,stroke:#B8B84A,color:#202124
  style payments fill:#FFFDE7,stroke:#B8B84A,color:#202124
```
