...
## Relationship Among Volatility Measures
...
## Duration as a Tangent-Line Approximation
...
def bond_price(yield_rate):
    ...
    return np.sum(cash_flows / (1 + yield_rate) ** periods)

def modified_duration(yield_rate):
    periods = np.arange(1, maturity + 1)
    cash_flows = np.full(maturity, coupon, dtype=float)
    cash_flows[-1] += face_value
    present_values = cash_flows / (1 + yield_rate) ** periods
    price = present_values.sum()
    macaulay_duration = np.sum(periods * present_values / price)
    return macaulay_duration / (1 + yield_rate)

yields = np.linspace(0.02, 0.12, 200)
prices = np.array([bond_price(y) for y in yields])
initial_price = bond_price(initial_yield)
duration = modified_duration(initial_yield)
tangent_prices = initial_price - duration * initial_price * (yields - initial_yield)
deviation_yields = np.array([0.035, 0.095])
actual_deviation_prices = np.array([bond_price(y) for y in deviation_yields])
tangent_deviation_prices = initial_price - duration * initial_price * (deviation_yields - initial_yield)

fig, ax = plt.subplots(figsize=(8, 5))
ax.plot(yields * 100, prices, linewidth=2.5, label="Actual price-yield curve")
ax.plot(yields * 100, tangent_prices, linestyle="--", linewidth=2, label="Duration tangent line")
ax.scatter(initial_yield * 100, initial_price, color="black", zorder=3)
ax.annotate(
    r"Initial yield $y^*$",
    xy=(initial_yield * 100, initial_price),
    xytext=(initial_yield * 100 + 0.6, initial_price + 7),
    arrowprops={"arrowstyle": "->", "linewidth": 1},
)
for y, actual_price, tangent_price in zip(
    deviation_yields,
    actual_deviation_prices,
    tangent_deviation_prices,
):
    ax.vlines(
        y * 100,
        tangent_price,
...
ax.annotate(
    "Duration approximation\nerror grows as yield\nmoves away from $y^*$",
    xy=(deviation_yields[0] * 100, actual_deviation_prices[0]),
    xytext=(2.1, actual_deviation_prices[0] + 7),
    arrowprops={"arrowstyle": "->", "linewidth": 1, "color": "tab:red"},
    color="tab:red",
)
ax.annotate(
    "Actual price remains\nabove tangent estimate",
    xy=(deviation_yields[1] * 100, actual_deviation_prices[1]),
    xytext=(8.1, actual_deviation_prices[1] + 9),
    arrowprops={"arrowstyle": "->", "linewidth": 1, "color": "tab:red"},
    color="tab:red",
)
ax.set_xlabel("Yield (%)")
ax.set_ylabel("Price per $100 par")
ax.set_title("Duration as a Tangent-Line Approximation")
ax.legend()
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
...
\frac{\Delta P}{P}
\approx
\frac{1}{P}\frac{dP}{dy}\Delta y
+
\frac{1}{2P}\frac{d^2P}{dy^2}(\Delta y)^2.
$$
...
## Convexity of the Hypothetical Bonds
...
yields = np.linspace(0.015, 0.105, 200)
yield_changes = yields - initial_yield
bond_a_prices = initial_price * (
    1 - modified_duration * yield_changes + 0.5 * convexity_a * yield_changes**2
)
bond_b_prices = initial_price * (
    1 - modified_duration * yield_changes + 0.5 * convexity_b * yield_changes**2
)

left_yield = 0.025
right_yield = 0.095
left_index = np.argmin(np.abs(yields - left_yield))
right_index = np.argmin(np.abs(yields - right_yield))

fig, ax = plt.subplots(figsize=(8, 5))
ax.plot(yields * 100, bond_a_prices, linewidth=2.3, label="Bond A: lower convexity")
ax.plot(yields * 100, bond_b_prices, linewidth=2.3, label="Bond B: higher convexity")
ax.scatter(initial_yield * 100, initial_price, color="black", zorder=4)
ax.annotate(
    "Same yield,\nsame price,\nsame duration",
    xy=(initial_yield * 100, initial_price),
    xytext=(4.6, 105.5),
...
for idx, label_y in [
    (left_index, "Greater upside\nwhen yields fall"),
    (right_index, "Smaller downside\nwhen yields rise"),
]:
    x = yields[idx] * 100
    y_low = bond_a_prices[idx]
    y_high = bond_b_prices[idx]
    ax.vlines(x, y_low, y_high, color="tab:red", linewidth=2, alpha=0.8)
    ax.scatter([x, x], [y_low, y_high], color="tab:red", s=25, zorder=4)
    ax.annotate(
        label_y,
        xy=(x, y_high),
        xytext=(x - 1.25 if x < initial_yield * 100 else x - 0.55, y_high + 1.4),
        arrowprops={"arrowstyle": "->", "linewidth": 1, "color": "tab:red"},
        color="tab:red",
    )
...
ax.set_xlabel("Yield (%)")
ax.set_ylabel("Price")
ax.set_title("Value of Convexity")
ax.legend()
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
...
HTML("""
<div id="convexity-widget" style="max-width: 980px; font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
  <div style="border: 1px solid #d6dde8; border-radius: 8px; padding: 12px; margin-bottom: 12px;">
    <div style="font-weight: 700; color: #1f77b4; margin-bottom: 8px;">Bond A</div>
    <label>Yield (%) <input id="cw-yield-a" type="number" min="0.5" max="15" step="0.25" value="6" style="width: 80px;"></label>
    <label style="margin-left: 12px;">Maturity <input id="cw-maturity-a" type="number" min="1" max="40" step="1" value="10" style="width: 70px;"></label>
    <div style="margin-top: 10px;">
      <label>Coupon: <span id="cw-coupon-a-label">2.0%</span></label>
      <input id="cw-coupon-a" type="range" min="0" max="12" step="0.25" value="2" style="width: 100%;">
    </div>
    <div id="cw-metrics-a" style="margin-top: 8px; font-size: 0.92rem;"></div>
  </div>
  <svg id="cw-chart" viewBox="0 0 900 520" width="100%" height="520" role="img" aria-label="Interactive price-yield convexity graph"></svg>
</div>

<script>
(function() {
  const face = 100;
  const ids = ["yield-a", "maturity-a", "coupon-a"];
  const controls = Object.fromEntries(ids.map(id => [id, document.getElementById("cw-" + id)]));
  ...
  function flows(couponPct, maturity) {
    const n = Math.max(1, Math.round(maturity));
    const coupon = face * couponPct / 100;
    const result = [];
    for (let t = 1; t <= n; t++) {
      result.push({t, cf: coupon + (t === n ? face : 0)});
    }
    return result;
  }

  function price(couponPct, maturity, yieldPct) {
    const y = yieldPct / 100;
    return flows(couponPct, maturity).reduce((total, row) => {
      return total + row.cf / Math.pow(1 + y, row.t);
    }, 0);
  }

  function metrics(couponPct, maturity, yieldPct) {
    const y = yieldPct / 100;
    const rows = flows(couponPct, maturity);
    const p = price(couponPct, maturity, yieldPct);
    const macaulay = rows.reduce((total, row) => {
      return total + row.t * row.cf / Math.pow(1 + y, row.t);
    }, 0) / p;
    const modified = macaulay / (1 + y);
    const convexity = rows.reduce((total, row) => {
      return total + row.t * (row.t + 1) * row.cf / Math.pow(1 + y, row.t + 2);
    }, 0) / p;
    return {price: p, macaulay, modified, convexity};
  }
  ...
  function render() {
    const a = {coupon: value("coupon-a"), maturity: value("maturity-a"), yield: value("yield-a"), color: "#1f77b4", label: "Bond A"};
    document.getElementById("cw-coupon-a-label").textContent = a.coupon.toFixed(1) + "%";

    const yieldGrid = Array.from({length: 140}, (_, i) => 1 + i * (13 / 139));
    const curve = {
      ...a,
      points: yieldGrid.map(y => ({x: y, y: price(a.coupon, a.maturity, y)})),
      metrics: metrics(a.coupon, a.maturity, a.yield)
    };

    const allPrices = curve.points.map(p => p.y);
    ...
    const xScale = x => left + (x - 1) / 13 * plotW;
    const yScale = y => top + (maxPrice + pad - y) / (maxPrice - minPrice + 2 * pad) * plotH;

    svg.innerHTML = "";
    line(left, top, left, top + plotH, {stroke: "#333", "stroke-width": 1});
    line(left, top + plotH, left + plotW, top + plotH, {stroke: "#333", "stroke-width": 1});
    for (let x = 2; x <= 14; x += 2) {
      const px = xScale(x);
      line(px, top, px, top + plotH, {stroke: "#e8edf3", "stroke-width": 1});
      text(px, top + plotH + 24, x + "%", {"font-size": 12, "text-anchor": "middle", fill: "#333"});
    }
    const yTicks = 5;
    for (let i = 0; i <= yTicks; i++) {
      const val = minPrice - pad + i * (maxPrice - minPrice + 2 * pad) / yTicks;
      const py = yScale(val);
      line(left, py, left + plotW, py, {stroke: "#e8edf3", "stroke-width": 1});
      text(left - 10, py + 4, val.toFixed(0), {"font-size": 12, "text-anchor": "end", fill: "#333"});
    }

    const d = curve.points.map((p, i) => `${i === 0 ? "M" : "L"} ${xScale(p.x)} ${yScale(p.y)}`).join(" ");
    svg.appendChild(make("path", {d, fill: "none", stroke: curve.color, "stroke-width": 3}));
    const currentPrice = price(curve.coupon, curve.maturity, curve.yield);
    svg.appendChild(make("circle", {cx: xScale(curve.yield), cy: yScale(currentPrice), r: 5, fill: curve.color}));
    text(xScale(curve.yield) + 8, yScale(currentPrice) - 8, "Bond A at selected yield", {"font-size": 13, fill: curve.color, "font-weight": 700});

    text(left + plotW / 2, height - 18, "Yield to maturity", {"font-size": 14, "text-anchor": "middle", fill: "#333"});
    text(20, top + plotH / 2, "Price per $100 par", {"font-size": 14, "text-anchor": "middle", fill: "#333", transform: `rotate(-90 20 ${top + plotH / 2})`});
    text(left, 22, "Price-yield relationship and convexity", {"font-size": 17, "font-weight": 700, fill: "#222"});

    document.getElementById("cw-metrics-a").innerHTML =
      `Price: <b>${curve.metrics.price.toFixed(2)}</b><br>` +
      `Macaulay duration: <b>${curve.metrics.macaulay.toFixed(2)}</b><br>` +
      `Modified duration: <b>${curve.metrics.modified.toFixed(2)}</b><br>` +
      `Convexity: <b>${curve.metrics.convexity.toFixed(2)}</b>`;
...
HTML("""
<div id="same-duration-widget" style="max-width: 980px; font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
  <div style="border: 1px solid #d6dde8; border-radius: 8px; padding: 12px; margin-bottom: 12px;">
    <div style="font-weight: 700; color: #1f77b4; margin-bottom: 8px;">Same-Duration Convexity Comparison</div>
    <label>Yield (%) <input id="sd-yield" type="number" min="0.5" max="15" step="0.25" value="6" style="width: 80px;"></label>
    <label style="margin-left: 12px;">Target modified duration <input id="sd-duration" type="number" min="1" max="30" step="0.25" value="8" style="width: 80px;"></label>
    <div style="margin-top: 10px;">
      <label>Coupon: <span id="sd-coupon-label">4.0%</span></label>
      <input id="sd-coupon" type="range" min="0" max="12" step="0.25" value="4" style="width: 100%;">
    </div>
    <div id="sd-metrics" style="margin-top: 8px; font-size: 0.92rem;"></div>
  </div>
  <svg id="sd-chart" viewBox="0 0 900 520" width="100%" height="520" role="img" aria-label="Interactive same-duration convexity graph"></svg>
</div>
...
(function() {
  const face = 100;
  const controls = {
    y: document.getElementById("sd-yield"),
    duration: document.getElementById("sd-duration"),
    coupon: document.getElementById("sd-coupon")
  };
  ...
  function closestMaturity(couponPct, yieldPct, targetDuration) {
    let best = {maturity: 1, error: Infinity, metrics: metrics(couponPct, 1, yieldPct)};
    for (let n = 1; n <= 80; n++) {
      const m = metrics(couponPct, n, yieldPct);
      const error = Math.abs(m.modified - targetDuration);
      if (error < best.error) {
        best = {maturity: n, error, metrics: m};
      }
    }
    return best;
  }
  ...
  function render() {
    const yieldPct = Number(controls.y.value);
    const targetDuration = Number(controls.duration.value);
    const couponPct = Number(controls.coupon.value);
    document.getElementById("sd-coupon-label").textContent = couponPct.toFixed(1) + "%";

    const match = closestMaturity(couponPct, yieldPct, targetDuration);
    const maturity = match.maturity;
    const m = match.metrics;
    const yieldGrid = Array.from({length: 140}, (_, i) => 1 + i * (13 / 139));
    const points = yieldGrid.map(y => ({x: y, y: price(couponPct, maturity, y)}));

    const allPrices = points.map(p => p.y);
    const minPrice = Math.min(...allPrices);
    const maxPrice = Math.max(...allPrices);
    const pad = 55;
    const width = 900;
    ...
    for (let i = 0; i <= 5; i++) {
      const val = minPrice - pad + i * (maxPrice - minPrice + 2 * pad) / 5;
      const py = yScale(val);
      line(left, py, left + plotW, py, {stroke: "#e8edf3", "stroke-width": 1});
      text(left - 10, py + 4, val.toFixed(0), {"font-size": 12, "text-anchor": "end", fill: "#333"});
    }
    ...
    svg.appendChild(make("path", {d, fill: "none", stroke: "#1f77b4", "stroke-width": 3}));
    const currentPrice = price(couponPct, maturity, yieldPct);
    svg.appendChild(make("circle", {cx: xScale(yieldPct), cy: yScale(currentPrice), r: 5, fill: "#1f77b4"}));
    text(xScale(yieldPct) + 8, yScale(currentPrice) - 8, "Selected yield", {"font-size": 13, fill: "#1f77b4", "font-weight": 700});

    text(left + plotW / 2, height - 18, "Yield to maturity", {"font-size": 14, "text-anchor": "middle", fill: "#333"});
    text(20, top + plotH / 2, "Price per $100 par", {"font-size": 14, "text-anchor": "middle", fill: "#333", transform: `rotate(-90 20 ${top + plotH / 2})`});
    text(left, 22, "Price-yield curve for selected coupon at target duration", {"font-size": 17, "font-weight": 700, fill: "#222"});

    document.getElementById("sd-metrics").innerHTML =
      `Implied maturity closest to target duration: <b>${maturity} years</b><br>` +
      `Price: <b>${m.price.toFixed(2)}</b><br>` +
      `Macaulay duration: <b>${m.macaulay.toFixed(2)}</b><br>` +
      `Modified duration: <b>${m.modified.toFixed(2)}</b> ` +
      `(target: ${targetDuration.toFixed(2)})<br>` +
      `Convexity: <b>${m.convexity.toFixed(2)}</b>`;
  }
...
::: {.callout-note title="Yield Curve Movements: Steepening, Flattening, Twists, and Curvature"}
...
**Key Insight**
Most yield curve movements can be decomposed into:
...
::: {.callout-note title="Example: Same Total Duration, Different Key Rate Exposures"}
...
portfolio_a = np.array([1.40, 0.30, 0.60, 2.00])
portfolio_b = np.array([0.20, 3.50, 0.50, 0.10])

scenarios = {
    "Parallel +100 bp": np.array([1.00, 1.00, 1.00, 1.00]),
    "5Y +100 bp": np.array([0.00, 1.00, 0.00, 0.00]),
    "30Y +100 bp": np.array([0.00, 0.00, 0.00, 1.00]),
    "Steepening": np.array([0.25, 0.50, 0.75, 1.00]),
}

loss_a = np.array([-np.dot(portfolio_a, shock) for shock in scenarios.values()])
loss_b = np.array([-np.dot(portfolio_b, shock) for shock in scenarios.values()])

fig, axes = plt.subplots(1, 2, figsize=(10, 4))

x = np.arange(len(key_rates))
width = 0.36
axes[0].bar(x - width / 2, portfolio_a, width, label="Portfolio A: barbell")
axes[0].bar(x + width / 2, portfolio_b, width, label="Portfolio B: bullet")
axes[0].set_xticks(x)
axes[0].set_xticklabels(key_rates)
axes[0].set_ylabel("Key rate duration")
axes[0].set_title("Key Rate Duration by Maturity")
axes[0].legend()
axes[0].grid(axis="y", alpha=0.3)

scenario_names = list(scenarios)
x2 = np.arange(len(scenario_names))
axes[1].bar(x2 - width / 2, loss_a, width, label="Portfolio A: barbell")
axes[1].bar(x2 + width / 2, loss_b, width, label="Portfolio B: bullet")
axes[1].axhline(0, color="black", linewidth=0.8)
axes[1].set_xticks(x2)
axes[1].set_xticklabels(scenario_names, rotation=25, ha="right")
axes[1].set_ylabel("Estimated price change (%)")
axes[1].set_title("Estimated Loss Under Different Yield Shocks")
axes[1].legend()
...
## Suggested Readings and Market Resources {.unnumbered}

- [U.S. Treasury – Interest Rate Statistics](https://home.treasury.gov/resource-center/data-chart-center/interest-rates) — Daily nominal and real Treasury curves provide observable shocks for duration, convexity, and key-rate sensitivity exercises.
- [Federal Reserve – Selected Interest Rates (H.15)](https://www.federalreserve.gov/releases/h15/) — Historical rates across maturities make it possible to examine parallel and nonparallel yield-curve movements with official data.
- [CME Group – U.S. Treasury Futures](https://www.cmegroup.com/markets/interest-rates/us-treasury.html) — Exchange contract specifications and market information connect duration exposure to widely used Treasury hedging instruments.
