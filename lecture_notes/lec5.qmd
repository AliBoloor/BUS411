...
Top fixed-income courses usually emphasize this separation: first value default-free cash flows, then add layers for credit, liquidity, taxes, and embedded options. MIT OCW fixed-income materials classify Treasuries, federal agency securities, and corporate securities as distinct building blocks of the debt market, while NYU fixed-income materials emphasize the zero-coupon curve, duration, convexity, option-adjusted spreads, and floating-rate note conventions as the tools used across those sectors.
...
security_map = pd.DataFrame(
    {
        ...
        "Why it exists": [
            ...
            "Floating-rate Treasury funding",
...
flowchart LR
  ...
  class B,D,E,H cash;
  class G reset;
  class Bills,Coupons,FRNs group;
...
The auction design solves two problems. First, Treasury needs to raise a large amount of money predictably. Second, investors need confidence that the allocation process is transparent. Noncompetitive bidding gives small investors access without requiring them to forecast the clearing yield. Competitive bidding lets large institutions reveal demand and set the market-clearing price.
...
In a single-price auction, a bidder who asked for a lower yield than the stop-out yield still receives the stop-out yield. This reduces the penalty for bidding aggressively and can improve auction participation.
:::
...
Clean-price quoting keeps market price movements easier to interpret. If bonds were quoted dirty, the quoted price would mechanically rise each day as accrued interest builds and then drop on the coupon date. Separating clean price from accrued interest lets traders see the change caused by yields and credit conditions rather than coupon-calendar mechanics.
