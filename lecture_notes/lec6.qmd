...
# ggplot2-style discrete colors for consistent bar charts.
R_RED = "#F8766D"
R_GREEN = "#00BA38"
R_BLUE = "#619CFF"
R_CYAN = "#00BFC4"
...
![Diagram](lec6_files/figure-latex/mermaid-figure-1.png){fig-align="center"}
...
![Diagram](lec6_files/figure-latex/mermaid-figure-5.png){fig-align="center"}
...
<!-- ![Diagram](lec6_files/figure-latex/mermaid-figure-3.png){fig-align="center"} -->
...
::: {.callout-note title="Example: Sector Active Weights"}
...
::: {.callout-note title="Example: Hidden Risk Inside a Cell"}
...
::: {.callout-note title="Comprehensive Example: Cell-Based Bond Portfolio Construction"}
Suppose a manager is asked to build a portfolio that tracks a broad investment-grade bond benchmark. The benchmark contains thousands of bonds, so buying every security is impractical. Instead, the manager uses a cell-based approach.

The benchmark is divided into cells based on four characteristics:
...
The total number of possible cells is:

$$
4 \times 3 \times 3 \times 2 = 72.
$$

Each benchmark bond is assigned to one cell. The manager then calculates the benchmark weight in each cell.

For example, suppose four cells are especially important:
...
A passive manager tries to match these weights closely. If the benchmark has 14% in intermediate A-rated corporate bonds, the portfolio should also hold about 14% in that cell.

The manager then chooses representative bonds within each cell:
...
The portfolio does not need to own every bond in the benchmark. It only needs to match the benchmark's main risk exposures well enough.

The manager checks whether the portfolio matches the benchmark on key risk measures:
...
These active differences are small, so predicted tracking error should be low.

Now suppose the manager has a mild active view: BBB corporate spreads look attractive. The manager may intentionally overweight the BBB corporate cell:
...
This creates an active credit tilt. The portfolio is no longer purely passive, but the tilt is controlled and easy to measure.

The key idea is that the cell-based approach reduces a huge bond-selection problem into a smaller matching problem:

$$
\text{match benchmark cell weights}
\quad
\Rightarrow
\quad
\text{match benchmark risk exposures}.
$$

A good cell-based portfolio should have:

- similar duration exposure;
- similar sector weights;
- similar credit-quality distribution;
- similar maturity structure;
- enough securities within each cell to avoid issuer concentration;
- low turnover and realistic transaction costs.

The approach is practical because fixed-income benchmarks often contain many bonds that are illiquid, unavailable, or too small to trade efficiently.
:::
...
1. Holdings and weights
2. Security-level factor exposures
3. Factor covariance and residual risk estimates
...
::: {.callout-tip title="Cross-Sectional Regression View"}
...
::: {.callout-note title="Broker Data Example"}
...
<!-- ![Diagram](lec6_files/figure-latex/mermaid-figure-4.png){fig-align="center"} -->
...
\text{Systematic TE}
=
\sqrt{(\Delta \mathbf{x})' \Sigma_F (\Delta \mathbf{x})}.
$$
...
::: {.callout-note title="Example: Building a 50-Bond Enhanced Index Portfolio"}
...
$$
\sum_i w_i\mathbf{1}_{\text{Treasury},i}\ge 30\%,
\quad
\sum_i w_i\mathbf{1}_{\text{Corporate},i}\le 40\%.
$$

It also tries to keep weighted-average duration and spread duration near the target:

$$
\sum_i w_iD_i \approx 5.0,
\quad
\sum_i w_iSD_i \approx 3.0.
$$
...
![Diagram](lec6_files/figure-latex/mermaid-figure-2.png){fig-align="center"}
...
- [SEC – Form N-PORT Data Sets](https://www.sec.gov/data-research/sec-markets-data/form-n-port-data-sets) — Public fund-holdings data support realistic portfolio, concentration, duration, and benchmark-comparison exercises.
- [FINRA – Fixed Income Data](https://www.finra.org/finra-data/fixed-income) — TRACE transaction data help incorporate liquidity, turnover, and implementation costs into portfolio construction.
- [Federal Reserve – Financial Accounts of the United States](https://www.federalreserve.gov/releases/z1/) — Official holdings and issuance tables show which sectors own and supply major fixed-income instruments, grounding benchmark weights in the real market.
