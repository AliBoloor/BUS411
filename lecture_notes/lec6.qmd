...
::: {.callout-note title="Long Example: Tracking Error from a Realistic Bond Mandate"}
...
The diagram highlights the key exam point: historical tracking error is a realized statistic, while predicted tracking error is a current-position risk estimate. A manager controls future risk with the second measure, not the first.
...
::: {.callout-note title="Why the Cell Diagram Matters"}
...
::: {.callout-note title="Key Rate Duration Example"}
The net 10-year key rate duration in the example is:

$$
1.39-1.51=-0.12.
$$

If the 10-year point alone rises by 50 basis points, holding other points constant, the approximate active return impact is:

$$
-(-0.12)\times0.0050=0.0006=6\text{ bps}.
$$

The portfolio outperforms because it has less exposure than the benchmark to that part of the curve. The sign matters: an underweight duration position benefits when that rate rises.
:::
...
The next example turns those real factor movements into approximate active returns. Suppose a portfolio has the following active exposures relative to its benchmark:

- net duration: +0.15 years;
- net investment-grade spread duration: +0.60 years;
- net high-yield spread duration: +0.20 years.

The approximate monthly active return from these factor mismatches is:

$$
\Delta R_{\text{active}}
\approx
-D_{\text{active}}\Delta y_{\text{Treasury}}
-SD_{\text{IG, active}}\Delta s_{\text{IG}}
-SD_{\text{HY, active}}\Delta s_{\text{HY}}.
$$

This is not a full return model, but it is the core intuition of factor-based risk attribution.

```{python}
#| label: fig-real-factor-active-return
#| fig-cap: "Approximate active return from real monthly factor changes and assumed active exposures."
#| echo: false
#| message: false
#| warning: false

factor_changes = monthly_fred[["DGS10", "BAMLC0A0CM", "BAMLH0A0HYM2"]].dropna().diff().dropna()
recent_changes = factor_changes.loc["2022-01-01":].copy()
if recent_changes.empty:
    recent_changes = factor_changes.tail(36).copy()

active_duration = 0.15
active_ig_spread_duration = 0.60
active_hy_spread_duration = 0.20

active_components = pd.DataFrame(index=recent_changes.index)
active_components["Treasury duration active return (bps)"] = -active_duration * recent_changes["DGS10"] * 100
active_components["IG spread active return (bps)"] = -active_ig_spread_duration * recent_changes["BAMLC0A0CM"] * 100
active_components["HY spread active return (bps)"] = -active_hy_spread_duration * recent_changes["BAMLH0A0HYM2"] * 100
active_components["Total modeled active return (bps)"] = active_components.sum(axis=1)

plot_components = active_components.tail(36)
fig, ax = plt.subplots()
ax.plot(plot_components.index, plot_components["Total modeled active return (bps)"], color="#111827", linewidth=2.5, label="Total modeled active return")
ax.bar(plot_components.index, plot_components["Treasury duration active return (bps)"], width=20, alpha=0.45, color="#2563eb", label="Treasury duration")
ax.bar(plot_components.index, plot_components["IG spread active return (bps)"], width=20, alpha=0.45, color="#16a34a", label="IG spread")
ax.bar(plot_components.index, plot_components["HY spread active return (bps)"], width=20, alpha=0.45, color="#dc2626", label="HY spread")
ax.axhline(0, color="#334155", linewidth=1)
ax.set_title("Factor Changes Create Active Returns")
ax.set_ylabel("Approximate active return (bps)")
...
factor_te_summary = pd.DataFrame(
    {
        "Measure": [
            "Monthly standard deviation of modeled active return",
            "Annualized modeled tracking error",
            "Worst monthly modeled active return",
            "Best monthly modeled active return",
        ],
        "Value (bps)": [
            active_components["Total modeled active return (bps)"].std(ddof=1),
            active_components["Total modeled active return (bps)"].std(ddof=1) * np.sqrt(12),
            active_components["Total modeled active return (bps)"].min(),
            active_components["Total modeled active return (bps)"].max(),
        ],
    }
)
factor_te_summary.round(1)
```

::: {.callout-tip title="Real-Data Intuition"}
The same active exposure can look harmless in quiet markets and painful in volatile markets. Tracking error depends on both the size of the active exposure and the volatility/correlation of the factors that move that exposure.
:::
...
| Is this risk cheap to trade? | Fix large liquid Treasury mismatches before small illiquid corporate mismatches. |
