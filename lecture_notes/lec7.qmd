...
- $\gamma=\tfrac{1}{2}$ (**Cox–Ingersoll–Ross model**): volatility is proportional to $\sqrt{r_t}$. Under the model's standard parameter restrictions, rates remain nonnegative because $\sigma_0 r_t^{1/2}$ goes to zero as $r_t$ approaches zero.
...
- time step: $\Delta t = 1/252$ years (one trading day)
...
where $q$ is the risk-neutral probability of the up state and $r$ is the one-period risk-free rate.
...
for key, (label, x, y) in nodes.items():
    ...
    ax.add_patch(box)
    ax.text(x + 0.725, y + 0.35, label, ha="center", va="center", fontsize=8.3)
...
for start, end, label in arrows:
    ...
    ax.text(mid_x, mid_y + 0.08, label, ha="center", va="center", fontsize=10)
...
1. Build possible future states of the underlying asset.
2. Compute the option payoff at the final nodes.
3. Move one step backward by discounting the expected future option value.
4. If the option can be exercised early, compare the continuation value with the exercise value at each node.
5. Continue backward until reaching today's value.
...
This is why the short-rate models discussed in this lecture are useful. They provide the future rate tree needed to value fixed-income options and bonds with embedded options.
...
Fabozzi, *Bond Markets: Analysis and Strategies*, Chapter 17.

### Real-Market Data and Resources {.unnumbered}

- [U.S. Treasury, Daily Treasury Par Yield Curve Rates](https://home.treasury.gov/resource-center/data-chart-center/interest-rates/TextView?type=daily_treasury_yield_curve): Observe the market term structure that arbitrage-free short-rate models are calibrated to reproduce.
- [Federal Reserve Bank of New York, Reference Rates](https://www.newyorkfed.org/markets/reference-rates): Access official SOFR and related overnight funding-rate data used to anchor the front end of U.S. interest-rate models.
- [FRED, Effective Federal Funds Rate](https://fred.stlouisfed.org/series/DFF): Download a long daily history of the policy-sensitive overnight rate for studying short-rate dynamics, volatility, and mean reversion.
- [Federal Reserve, Selected Interest Rates (H.15)](https://www.federalreserve.gov/releases/h15/): Compare current and historical Treasury, policy, and private short-term rates in an official weekly release.
