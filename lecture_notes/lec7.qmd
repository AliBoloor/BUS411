...
$$
P = \sum_i CF_i \times DF(t_i),
$$

where $CF_i$ is the cash flow at date $t_i$ and $DF(t_i)$ is the discount factor for that date. The discount factors can be bootstrapped from Treasury prices, swap rates, or other benchmark instruments. No model of how future interest rates might move is required.
...
| Feature | Plain fixed‑rate bond | Embedded‑option bond or rate derivative |
|---|---|---|
| Cash flows | Fixed or mostly fixed | Depend on future rates and exercise decisions |
| Main valuation input | Bootstrapped discount curve | Discount curve plus rate dynamics |
| Typical method | Discount each cash flow | No‑arbitrage tree, lattice, or Monte Carlo |
| Model of future rates needed? | Usually no | Yes |
...
A lattice or tree is a discrete version of the same idea. Starting from today’s short rate $r_0$, rates branch into possible future values. Each path has its own sequence of short rates and therefore its own discount factor. For an option‑embedded bond, the model also checks whether exercise is optimal at each node.

```{python}
#| label: fig-short-rate-tree
#| fig-cap: "A simple short‑rate tree: each path implies a discount factor"
#| echo: false
import matplotlib.pyplot as plt
...
for label, x, y in nodes.values():
    ax.scatter(x, y, s=900, color="#E8F1FA", edgecolor="black", zorder=3)
    ax.text(x, y, label, ha="center", va="center", fontsize=12, zorder=4)

ax.text(1, -2.75, "Example path discount factor:  $e^{-r_0\\Delta t}e^{-r_d\\Delta t}e^{-r_{dd}\\Delta t}$",
