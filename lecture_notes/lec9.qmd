...
Thus, a 1-basis-point change in the implied rate corresponds to about $25 per contract. Since the price is $100 - \text{rate}$, a higher SOFR futures price means a lower implied SOFR rate.
...
```{python}
#| label: fig-short-rate-futures-comparison
#| fig-cap: "Short-rate futures prices are quoted as 100 minus the implied short rate"
import numpy as np
import matplotlib.pyplot as plt

implied_rates = np.linspace(3.0, 6.0, 61)
futures_prices = 100 - implied_rates

plt.figure(figsize=(7, 3.4))
plt.plot(implied_rates, futures_prices, linewidth=2, color="#4C78A8")
plt.scatter([4.80, 4.375], [95.20, 95.625], color=["#F58518", "#72B7B2"], zorder=3)
plt.text(4.83, 95.08, "SOFR example", fontsize=8)
plt.text(4.03, 95.72, "Fed Funds example", fontsize=8)
plt.xlabel("Implied short rate (%)")
plt.ylabel("Futures price")
plt.title("Index price moves inversely with the implied rate")
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()
```
...
If the futures price is 107, the cash-and-carry trade has no initial net investment because the bond purchase is financed by borrowing. At delivery:

| Cash flow item | Amount |
|---|---:|
| Futures delivery price | 107 |
| Accrued coupon interest | 3 |
| Total proceeds | 110 |
| Loan principal repayment | -100 |
| Loan interest | -2 |
| Arbitrage profit | 8 |

The high futures price creates a profit for buying the cash bond and selling the futures.
:::

::: {.callout-note title="Example 6: When the Futures Price Is Too Low"}
If the futures price is 92, the reverse cash-and-carry trade is profitable. The trader shorts the bond, invests the proceeds, and buys the futures. At delivery:

| Cash flow item | Amount |
|---|---:|
| Investment principal received | 100 |
| Investment interest | 2 |
| Total proceeds | 102 |
| Futures delivery price | -92 |
| Accrued coupon interest paid | -3 |
| Arbitrage profit | 7 |
