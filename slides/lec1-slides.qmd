...
These are the lecture’s descriptive conventions, not universal boundaries.
...
![](../lecture_notes/images/lec1-yield-curve.svg){fig-alt="Hypothetical upward-sloping yield curve with maturity in years and annual yield in percent." height="460"}
...
Example: if the coupon is $10\%-2r_t$, a rise in $r_t$ from 3% to 4% lowers the coupon from 4% to 2%, before any contractual floor.
...
For example, 2% indexation increases a \$1,000 principal amount to \$1,020. A 1% annual coupon rate then implies **\$10.20** in annual coupon payments.
...
## Different structures for different investor needs

- **Mortgage pass-through:** distributes a proportional share of available pool payments.
- **Collateralized mortgage obligation (CMO):** allocates payments among tranches with different repayment priorities and expected lives.
- **Interest-only (IO) claim:** receives designated interest cash flows.
- **Principal-only (PO) claim:** receives designated principal cash flows.

Different investors can choose different timing and prepayment exposures. Structuring does not create additional borrower income or remove common default risk.
...
- **Risk risk:** the investor misunderstands the security’s risks or structure.
